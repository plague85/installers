...
then
	...
	if ! grep -q '#Percona' "/etc/apt/sources.list" ; then
        	...
        	echo "deb http://repo.percona.com/apt raring main" | tee -a /etc/apt/sources.list
	        echo "deb-src http://repo.percona.com/apt raring main" | tee -a /etc/apt/sources.list
	fi
	echo -e "Updating Apt Catalog\033[0m"
        disablemaria
        disablepostgresql
...
wget http://mediaarea.net/download/binary/libzen0/0.4.29/libzen0_0.4.29-1_amd64.xUbuntu_12.10.deb
wget http://mediaarea.net/download/binary/libmediainfo0/0.7.64/libmediainfo0_0.7.64-1_amd64.xUbuntu_12.10.deb
wget http://mediaarea.net/download/binary/mediainfo/0.7.64/mediainfo_0.7.64-1_amd64.Debian_6.0.deb
dpkg -i libzen0*
...
if [[ $EXTRAS == "y" ]]; then
	apt-get install -qq nmon mytop iftop bwm-ng vnstat atop iotop ifstat htop pastebinit pigz iperf geany geany-plugins-common geany-plugins geany-plugin-spellcheck ttf-mscorefonts-installer diffuse tinyca meld tmux unrar p7zip-full tmux make screen
...
apt-get autoclean
apt-get install -qq tmux python-setuptools python-mysqldb
